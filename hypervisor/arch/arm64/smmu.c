...
void iommu_prepare_shutdown(void)
{
	...
	if (!smmu->base)
		return;
	/* Program SMMU to let All DMA bypass SMMU */
	/* Turn the thing off */
	mmio_write32(ARM_SMMU_GR0_NS(smmu) + ARM_SMMU_GR0_sCR0, sCR0_CLIENTPD);
}

static unsigned int iommu_count_units(void)
{
...
static unsigned int arm_smmu_mmio_count_regions(struct cell *cell)
{
...
#define for_each_smmu_sid(sid, config, counter)				\
	for ((sid) = jailhouse_cell_sid(config), (counter) = 0;	\
	     (counter) < (config)->num_smmu_sids;			\
	     (sid)++, (counter)++)
...
static int arm_smmu_cell_init(struct cell *cell)
{
	...
	const struct jailhouse_smmu_sid *sid;
	unsigned int n;
	int ret, idx;
	...
	for_each_smmu_sid(sid, cell->config, n) {
		ret = arm_smmu_find_sme(sid->sid, sid->sid_mask);
		if (ret < 0)
...
void iommu_config_commit(struct cell *cell_added_removed)
{
	if (cell_added_removed) {
		printk("%s %s\n", __func__, cell_added_removed->config->name);
		/* TODO: Add interrupt? */
	}

	if (cell_added_removed == &root_cell) {
		/*
		 * Parse the root cell configuration
		 * and Program SMMU
		 */
		arm_smmu_cell_init(&root_cell);
