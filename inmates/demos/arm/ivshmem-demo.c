...
void inmate_main(void)
{
       ...
       volatile char *shmem;
       int ndevices = 0;
       unsigned int ivshmem_irq = comm_region->ivshmem_irq;

       gic_setup(handle_irq);

       map_range((void*)comm_region->pci_cfg_base, comm_region->pci_cfg_size, MAP_UNCACHED);
       ...
       while ((ndevices < MAX_NDEV) &&
              (-1 != (bdf = pci_find_device(VENDORID, DEVICEID, bdf)))) {
               ...
               printk("IVSHMEM: mapped shmem and bars, got position %x\n",
                      get_ivpos(d));
